        ...
        <div>
            ...
            <div style={{ marginBottom: "8px" }}>
                {suggestedFactors.map((sf) => (
                    <button
                        key={sf.name}
                        style={{ backgroundColor: "#8B5CF6", color: "#fff", marginRight: "6px" }}
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", marginTop: "16px" }}>
                    ...
                    {currentList.map((item) => (
                        <tr key={item.factorName}>
                            ...
                            {founders.map((fn) => (
                                <td key={fn}>
                                    <input
                                        ...
                                        onChange={(e) => handleAllocationChange(item.factorName, fn, e.target.value)}
                                    />
                                    %
                                </td>
                            ))}
                            <td>
                                <button
                                    style={{ background: "none", border: "none", color: "red", fontSize: "20px" }}
            ...
            {disclaimer && <p style={{ marginTop: "8px", color: "#555" }}>{disclaimer}</p>}
        </div>
    );
}
